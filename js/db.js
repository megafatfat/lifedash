...
const db = {
  ...
  async getTransactions() {
    ...
    try {
      const { data, error } = await this.client.from('transactions').select('*').order('date', { ascending: false });
  ...
  async getBudget() {
    ...
    try {
      const { data, error } = await this.client.from('budgets').select('*').limit(1).single();
  ...
  async saveBudget(amount) {
    const payload = { budget_amount: amount };
    if (!this.isReady()) {
      this.setLocal('lifedash_budget', { budget: amount });
      return;
    }
    try {
      const { data: existing } = await this.client.from('budgets').select('id').limit(1).single();
      if (existing) {
        await this.client.from('budgets').update(payload).eq('id', existing.id);
      } else {
  ...
  async getPomodoroStats() {
    ...
    try {
      const { data, error } = await this.client.from('pomodoro_stats').select('*').limit(1).single();
      if (error && error.code !== 'PGRST116') throw error;
  ...
  async savePomodoroStats(stats) {
    ...
    try {
      const { data: existing } = await this.client.from('pomodoro_stats').select('id').limit(1).single();
      const payload = { total_completed: stats.totalCompleted, total_focus_minutes: stats.totalFocusMinutes };
      if (existing) {
        await this.client.from('pomodoro_stats').update(payload).eq('id', existing.id);
      } else {
  ...
  async getClockCities() {
    ...
    try {
      const { data, error } = await this.client.from('clock_cities').select('*').order('sort_order');
      if (error) throw error;
  ...
  async saveClockCities(cities) {
    ...
    try {
      await this.client.from('clock_cities').delete().neq('id', 0);
      if (cities.length > 0) {
